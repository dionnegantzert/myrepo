...
### New tab
#### Comparing the Composers typical songs
In this section we will look into different self-similarity matrices of three typical songs. The first track will be "The Throne Room and End Title" by John Williams. The second track will be "Ahsoka and the Martez Sisters" by Kevin Kiner and the last track will be "Mando Rescue" by Ludwig Goransson.

#### John Williams: The Throne Room and End Title
...
In this graph you can see a clear difference between the self-similarity of the chroma features and the timbre features of "The Throne Room and End Title" by John Williams. The lighter the colors are, the more similar the features. In this graph it is clear that the chroma features around the 200 are more similar than the others. Regarding timbre, not only the 200 but also around 70 it is noticeable that the features are more similar. 

#### Kevin Kiner: Ahsoka and the Martez Sisters
```{r, echo = FALSE, message = FALSE, fig.align='center'}
jedi_temple <-
  get_tidy_audio_analysis("0hd8rk3WUINYVbdfsu1CqH") %>%
  compmus_align(bars, segments) %>%
  select(bars) %>%
  unnest(bars) %>%
  mutate(
...
In this graph you can see a clear difference between the self-similarity of the chroma features and the timbre features of "Ahsoka and the Martez Sisters" by Kevin Kiner. The lighter the colors are, the more similar the features. Regarding the chroma features you see a huge difference between this track and the track by Williams. Where Williams chroma features were more similar around the 200, here the chroma features are highly similar to each other in general. Usually a self-similarity matrix is very similar in a few points but here in this song we can see that the chroma features are much alike. Regarding timbre, this looks almost the same as Williams' track above. Except it is more smooth. **Why?**

#### Ludwig Goransson: Mando Rescue
```{r, echo = FALSE, message = FALSE, fig.align='center'}
mando_rescue <-
  get_tidy_audio_analysis("2KDvhi03HTPCWXNMPvLtr9") %>%
  compmus_align(bars, segments) %>%
  select(bars) %>%
  unnest(bars) %>%
  mutate(
...
In this graph you can see a clear difference between the self-similarity of the chroma features and the timbre features of "Mando Rescue" by Ludwig Goransson. The lighter the colors are, the more similar the features. The first noticeable difference is that this graph is much smoother than the other two tracks by Kiner and Williams. **Why? What does this mean?**. Another difference is that with Williams' track the chroma features are more similar around 200, while here it is around 10 and 120. It is the exact opposite of Williams' track. **How do I explain this well?**
